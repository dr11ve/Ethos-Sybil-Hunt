...
ETHOS_CLIENT_NAME = os.getenv("ETHOS_CLIENT_NAME", "ethos-sybil-bot")
...
def is_ethos_checkable_address(address: Optional[str]) -> bool:
...
def check_ethos_account(client: httpx.Client, address: str) -> Dict[str, Any]:
    ...
    if not is_ethos_checkable_address(normalized):
        return {
            "address": normalized or str(address),
            "exists": False,
            "status": None,
            "username": None,
            "profile_url": None,
            "error": "invalid_for_ethos",
        }

    url = f"{ETHOS_SINGLE_USER_BY_ADDRESS_URL}/{normalized}"
    result = http_request_with_retries(
        client,
        "GET",
        url,
        op_name=f"ethos:by_address:{normalized}",
        max_retries=MAX_RETRIES,
        retry_on_statuses=ETHOS_RETRYABLE_STATUSES,
        headers={"X-Ethos-Client": ETHOS_CLIENT_NAME},
    )

    if not result["ok"]:
        return {"address": normalized, "exists": None, "status": None, "username": None, "profile_url": None, "error": result.get("error") or "request_failed"}

    resp = result["response"]
    if resp is None:
        return {"address": normalized, "exists": None, "status": None, "username": None, "profile_url": None, "error": "empty_response"}

    if resp.status_code == 404:
        return {"address": normalized, "exists": False, "status": None, "username": None, "profile_url": None, "error": None}

    if resp.status_code == 400:
        return {"address": normalized, "exists": False, "status": None, "username": None, "profile_url": None, "error": "invalid_for_ethos"}

    if resp.status_code != 200:
        return {"address": normalized, "exists": None, "status": None, "username": None, "profile_url": None, "error": f"http_{resp.status_code}"}

    try:
        data = resp.json()
    except Exception as e:
        return {"address": normalized, "exists": None, "status": None, "username": None, "profile_url": None, "error": f"invalid_json: {e}"}
...
def check_ethos_accounts_bulk(client: httpx.Client, addresses: List[str]) -> List[Dict[str, Any]]:
    normalized_addresses = [normalize_address(a) for a in addresses if normalize_address(a)]
    unique_addresses = list(dict.fromkeys([a for a in normalized_addresses if a]))
    ...
    filtered_bad: List[str] = []

    for addr in unique_addresses:
        if is_ethos_checkable_address(addr):
            valid_addresses.append(addr)
        else:
            filtered_bad.append(addr)
            results_by_address[addr] = {
                ...
                "username": None,
                "profile_url": None,
                "error": "filtered_invalid_address",
            }

    if filtered_bad:
        log(f"[ethos] filtered {len(filtered_bad)} invalid addresses before bulk check")
        for addr in filtered_bad[:10]:
            log(f"[ethos] filtered invalid address: {addr}")

    for i in range(0, len(valid_addresses), ETHOS_BULK_MAX_ADDRESSES):
        chunk = valid_addresses[i:i + ETHOS_BULK_MAX_ADDRESSES]
        ...
        if not result["ok"] or result.get("status_code") == 400:
            err = result.get("error") or f"http_{result.get('status_code')}"
            log(f"[ethos] bulk failed for chunk, fallback to single checks: {err}")
            for addr in chunk:
    ...
    return [results_by_address[addr] for addr in unique_addresses if addr in results_by_address]
...
def analyze_wallet(client: httpx.Client, target: str, chat_id: str) -> None:
    ...
    related_addresses = [a for a in related_addresses if is_ethos_checkable_address(a)]
    ...
    bulk_related = [a for a in related_addresses if a != target]
    log(f"Checking {1 + len(bulk_related)} addresses on Ethos Network (main via single, related via bulk)...")
    ethos_all = check_ethos_accounts_bulk(client, bulk_related)
    ethos_by_addr = {item["address"]: item for item in ethos_all}

    linked_ethos_results = [ethos_by_addr[a] for a in bulk_related if a in ethos_by_addr]
